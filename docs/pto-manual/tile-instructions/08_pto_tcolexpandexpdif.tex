\section{\texttt{TCOLEXPANDEXPDIF}}
\noindent\textbf{Descriptor profile.} Vec entry 8, tile catalog opcode \texttt{0x1013}. Descriptor bundle: \texttt{B.OP + B.DIM + B.IOT}; WO/WM sites: 9; VEC beat-level sites: 9; ROM bytes: 72.\par\smallskip
\TileInstructionEncoding{figs/encoding/tile_instruction_entries/entry_08_pto_tcolexpandexpdif_th0.pdf}{figs/encoding/tile_instruction_entries/entry_08_pto_tcolexpandexpdif_th2.pdf}{0x1013}{8}{B.OP + B.DIM + B.IOT}
\paragraph{Bundled descriptor (PTO-ISA header).}
\begin{description}[leftmargin=0.18\linewidth,style=nextline]
\item[Bundle] \texttt{B.OP + B.DIM + B.IOT}. \texttt{B.OP.desc\_count} = 2; \texttt{B.OP.rom\_entry\_id} = 8.
\item[Assembly] 0: \texttt{B.OP} selects \texttt{TCOLEXPANDEXPDIF}, carries element format/variant, declares \texttt{desc\_count}, and names the ROM entry. 1: \texttt{B.DIM} supplies column-axis reduction or column-broadcast bounds. 2: \texttt{B.IOT} binds architectural tile operands. Across all \texttt{B.IOT} descriptors: tile inputs \texttt{src0}, \texttt{src1}, mask inputs none, and outputs \texttt{dst}.
...
def rom_tcolexpandexpdif_f16(src0, src1, dst):
...
def rom_tcolexpandexpdif_f32(src0, src1, dst):
    dtype = dst.element_type
    valid_rows, valid_cols = dst.valid_shape
