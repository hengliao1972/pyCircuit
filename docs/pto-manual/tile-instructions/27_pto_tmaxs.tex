\subsection{\texttt{TMAXS}}
\noindent\textbf{Bundle encoding.} UOP entry 27, opcode \texttt{0x1035}. Bundle: \texttt{B.OP + B.ARG + B.IOR + B.IOTI}. \texttt{B.OP.Opcode}=\texttt{0x1035}. \texttt{B.IOTI}: selector=\texttt{111 last}, \texttt{SrcTile0}=\texttt{src}, \texttt{SrcTile1}=\texttt{absent}, \texttt{DstTile}=\texttt{dst}, \texttt{SizeCode}=profile tile size. \texttt{B.IOR} imports scalar operand(s): \texttt{scalar}.\par\smallskip
{\small
\paragraph{PTO ISA description.}
Elementwise max of a tile and a scalar: \texttt{max(src, scalar)}.
\paragraph{Example.}
Tile [1, 4, 2] max scalar 3 = [3, 4, 3].
\par\smallskip
...
\paragraph{Notes.}
The scalar is imported via \texttt{B.IOR}. NaN handling same as \texttt{TMAX}.
