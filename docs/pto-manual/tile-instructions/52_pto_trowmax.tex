\subsection{\texttt{TROWMAX}}
\noindent\textbf{Bundle encoding.} UOP entry 52, opcode \texttt{0x1052}. Bundle: \texttt{B.OP + B.ARG + B.DIM + B.IOTI}. \texttt{B.OP.Opcode}=\texttt{0x1052}. \texttt{B.IOTI}: selector=\texttt{111 last}, \texttt{SrcTile0}=\texttt{src}, \texttt{SrcTile1}=\texttt{absent}, \texttt{DstTile}=\texttt{dst}, \texttt{SizeCode}=profile tile size. \texttt{B.DIM} supplies row-axis reduction or row-broadcast bounds. Scratch operand(s) \texttt{tmp} are implementation-local and are not encoded in the CPU-visible bundle.\par\smallskip
{\small
\paragraph{PTO ISA description.}
Reduce each row by taking the maximum across columns.
\paragraph{Example.}
Source row [3, 1, 7, 2]: output[0] = 7.
\par\smallskip
...
\paragraph{Notes.}
Output tile has shape $[R_{src}, 1]$. For FP types, NaN propagates.
