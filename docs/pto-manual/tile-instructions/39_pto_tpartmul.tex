\subsection{\texttt{TPARTMUL}}
\noindent\textbf{Bundle encoding.} UOP entry 39, opcode \texttt{0x1044}. Bundle: \texttt{B.OP + B.ARG + B.ATTR + B.IOTI}. \texttt{B.OP.Opcode}=\texttt{0x1044}. \texttt{B.IOTI}: selector=\texttt{111 last}, \texttt{SrcTile0}=\texttt{src0}, \texttt{SrcTile1}=\texttt{src1}, \texttt{DstTile}=\texttt{dst}, \texttt{SizeCode}=profile tile size. \texttt{B.ATTR} carries partial-valid-region policy.\par\smallskip
{\small
...
\paragraph{Example.}
Same pattern as \texttt{TPARTADD} $+$ but with multiplication instead of addition.
\par\smallskip
...
\paragraph{Notes.}
Integer multiplication wraps. Saturated integer types saturate.
...
def uop_tpartmul(src0, src1, dst):
