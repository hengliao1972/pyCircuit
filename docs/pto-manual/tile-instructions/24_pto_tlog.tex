\subsection{\texttt{TLOG}}
\noindent\textbf{Bundle encoding.} UOP entry 24, opcode \texttt{0x1030}. Bundle: \texttt{B.OP + B.ARG + B.IOTI}. \texttt{B.OP.Opcode}=\texttt{0x1030}. \texttt{B.IOTI}: selector=\texttt{111 last}, \texttt{SrcTile0}=\texttt{src}, \texttt{SrcTile1}=\texttt{absent}, \texttt{DstTile}=\texttt{dst}, \texttt{SizeCode}=profile tile size.\par\smallskip
{\small
\paragraph{PTO ISA description.}
Elementwise natural logarithm of a tile.
\paragraph{Example.}
Tile elements [1, e, 1/e] give [0, 1, -1].
\par\smallskip
...
\paragraph{Notes.}
Accuracy is implementation-defined. ln(0) = -Inf. ln(negative) = NaN. For FP types, NaN propagates. The VARIANT field may select different approximation algorithms.
