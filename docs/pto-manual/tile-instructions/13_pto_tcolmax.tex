\subsection{\texttt{TCOLMAX}}
\noindent\textbf{Bundle encoding.} UOP entry 13, opcode \texttt{0x1018}. Bundle: \texttt{B.OP + B.ARG + B.DIM + B.IOTI}. \texttt{B.OP.Opcode}=\texttt{0x1018}. \texttt{B.IOTI}: selector=\texttt{111 last}, \texttt{SrcTile0}=\texttt{src}, \texttt{SrcTile1}=\texttt{absent}, \texttt{DstTile}=\texttt{dst}, \texttt{SizeCode}=profile tile size. \texttt{B.DIM} supplies column-axis reduction or column-broadcast bounds.\par\smallskip
{\small
\paragraph{PTO ISA description.}
Reduce each column by taking the maximum across rows.
\paragraph{Example.}
Source 4x3 tile. Output[0,0] = max(src[0,0], src[1,0], src[2,0], src[3,0]).
\par\smallskip
...
\paragraph{Notes.}
The output tile has shape $[1, C_{src}]$. For FP types, NaN propagates: if any input in the column is NaN the result is NaN.
