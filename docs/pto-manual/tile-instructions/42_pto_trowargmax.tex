\subsection{\texttt{TROWARGMAX}}
\noindent\textbf{Bundle encoding.} UOP entry 42, opcode \texttt{0xF000}. Bundle: \texttt{B.OP + B.ARG + B.DIM + B.IOTI}. \texttt{B.OP.Opcode}=\texttt{0xF000}. \texttt{B.IOTI}: selector=\texttt{111 last}, \texttt{SrcTile0}=\texttt{src}, \texttt{SrcTile1}=\texttt{absent}, \texttt{DstTile}=\texttt{dst}, \texttt{SizeCode}=profile tile size. \texttt{B.DIM} supplies row-axis reduction or row-broadcast bounds. Scratch operand(s) \texttt{tmp} are implementation-local and are not encoded in the CPU-visible bundle.\par\smallskip
{\small
\paragraph{PTO ISA description.}
Get the column index of the maximum element for each row.
\paragraph{Example.}
Source row [3, 1, 7, 2]: output row[0] = 2 (0-indexed column of max value 7).
\par\smallskip
\paragraph{PTO ISA operation.}
Let \texttt{R = src.GetValidRow()} and \texttt{C = src.GetValidCol()}. For \texttt{0 \textless{}= i \textless{} R}:

\[
\mathrm{dst}_{i,0} = \operatorname*{arg\,max}_{0 \le j < C}\; \mathrm{src}_{i,j}
\]

\noindent where the result is the column index $j$ that maximises $\mathrm{src}_{i,j}$.
}\par\smallskip
\paragraph{Notes.}
The output element type is declared by \texttt{B.OP.DataType}, which should be an integer type (typically \texttt{i32} or \texttt{ui32}). Ties resolve to the smallest column index. The microcode body maintains both a value accumulator and an index accumulator, updating the index when a strictly larger value is found.
...
def uop_trowargmax(src, tmp, dst):
