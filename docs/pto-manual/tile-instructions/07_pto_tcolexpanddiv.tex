\subsection{\texttt{TCOLEXPANDDIV}}
\noindent\textbf{Bundle encoding.} UOP entry 7, opcode \texttt{0x1012}. Bundle: \texttt{B.OP + B.ARG + B.DIM + B.IOTI}. \texttt{B.OP.Opcode}=\texttt{0x1012}. \texttt{B.IOTI}: selector=\texttt{111 last}, \texttt{SrcTile0}=\texttt{src0}, \texttt{SrcTile1}=\texttt{src1}, \texttt{DstTile}=\texttt{dst}, \texttt{SizeCode}=profile tile size. \texttt{B.DIM} supplies column-axis reduction or column-broadcast bounds.\par\smallskip
{\small
...
\paragraph{Example.}
dst[:,j] = tile[:,j] / src[0,j] for each column j. Division by zero produces Inf.
\par\smallskip
...
\paragraph{Notes.}
The VARIANT field selects the FP rounding mode. IEEE 754 Inf handling applies.
...
def uop_tcolexpanddiv(src0, src1, dst):
