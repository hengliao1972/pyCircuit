\section{\texttt{TSHRS}}
\noindent\textbf{Descriptor profile.} Vec entry 62, tile catalog opcode \texttt{0x1062}. Descriptor bundle: \texttt{B.OP + B.IOR + B.IOT}; WO/WM sites: 3; VEC beat-level sites: 3; ROM bytes: 24.\par\smallskip
\TileInstructionEncoding{figs/encoding/tile_instruction_entries/entry_62_pto_tshrs_th0.pdf}{figs/encoding/tile_instruction_entries/entry_62_pto_tshrs_th2.pdf}{0x1062}{62}{B.OP + B.IOR + B.IOT}
\paragraph{Bundled descriptor (PTO-ISA header).}
\begin{description}[leftmargin=0.18\linewidth,style=nextline]
\item[Bundle] \texttt{B.OP + B.IOR + B.IOT}. \texttt{B.OP.desc\_count} = 2; \texttt{B.OP.rom\_entry\_id} = 62.
\item[Assembly] 0: \texttt{B.OP} selects \texttt{TSHRS}, carries element format/variant, declares \texttt{desc\_count}, and names the ROM entry. 1: \texttt{B.IOR} imports scalar operand(s) \texttt{scalar} through 5-bit scalar register IDs. 2: \texttt{B.IOT} binds architectural tile operands. Across all \texttt{B.IOT} descriptors: tile inputs \texttt{src}, mask inputs none, and outputs \texttt{dst}.
...
\midrule
0 & \texttt{u\_vec.vlds} & \texttt{opc=0x17, x=0, fl=mem, seq=0, kind=b} & \texttt{entry=62, cnt=2, res=vec, var=0} \\
1 & \texttt{u\_vec.vshrs} & \texttt{opc=0x28, x=0, fl=alu, seq=1, kind=b} & \texttt{entry=62, cnt=2, res=vec, var=0} \\
2 & \texttt{u\_vec.vsts} & \texttt{opc=0x2A, x=0, fl=mem, seq=2, kind=b} & \texttt{entry=62, cnt=2, res=vec, var=0} \\
\bottomrule
\end{longtable}
}
\paragraph{ROM body DSL.}
\begin{lstlisting}[style=ptocode]
def rom_tshrs(src, scalar, dst):
