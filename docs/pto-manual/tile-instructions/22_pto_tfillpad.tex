\subsection{\texttt{TFILLPAD}}
\noindent\textbf{Bundle encoding.} UOP entry 22, opcode \texttt{0x1023}. Bundle: \texttt{B.OP + B.ARG + B.ATTR + B.IOTI}. \texttt{B.OP.Opcode}=\texttt{0x1023}. \texttt{B.IOTI}: selector=\texttt{111 last}, \texttt{SrcTile0}=\texttt{src}, \texttt{SrcTile1}=\texttt{absent}, \texttt{DstTile}=\texttt{dst}, \texttt{SizeCode}=profile tile size. \texttt{B.ATTR} carries pad/fill and valid-region policy.\par\smallskip
...
{\small
The pad value comes from the fill parameter.
