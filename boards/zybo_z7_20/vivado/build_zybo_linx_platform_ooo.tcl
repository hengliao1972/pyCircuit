...
# (OOO core: janus_bcc_ooo_pyc).
#
# Usage:
#   vivado -mode batch -source boards/zybo_z7_20/vivado/build_zybo_linx_platform_ooo.tcl
#
# Optional env vars:
...
set build_dir  [file normalize [file join $script_dir build_linx_platform_ooo]]
file mkdir $build_dir

set proj_name "zybo_linx_platform_ooo"
...
add_files -norecurse [file join $repo_root boards zybo_z7_20 rtl linx_platform_ooo_axi.sv]
add_files -norecurse [file join $repo_root janus generated janus_bcc_ooo_pyc janus_bcc_ooo_pyc.v]
...
set plat [create_bd_cell -type module -reference linx_platform_ooo_axi linx_plat_0]
...
if {[file exists $bit_path]} {
  file copy -force $bit_path [file join $build_dir linx_platform_ooo.bit]
  puts "Wrote: [file join $build_dir linx_platform_ooo.bit]"
} else {
  puts "Expected bitstream not found: $bit_path"
}

set xsa_path [file normalize [file join $build_dir linx_platform_ooo.xsa]]
catch {
